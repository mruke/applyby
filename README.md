...
|---|---|---|---|
| 1 | Backend foundation | Initialize the Go backend module, package structure, and first test workflow. | Planned |
| 2 | Domain model | Define the core job application, company, contact, reminder, document, and activity concepts. | Planned |
| 3 | Application lifecycle | Implement validated application status transitions using an explicit lifecycle model. | Planned |
| 4 | Application workflows | Add use cases for creating, updating, listing, and managing applications while keeping workflows separate from storage and API code. | Planned |
| 5 | PostgreSQL persistence | Add relational schema, migrations, repositories, constraints, and integration tests for durable storage. | Planned |
| 6 | Search and reminders | Add indexed search/filter behavior and reminder prioritization for performance-conscious job-search workflows. | Planned |
| 7 | HTTP API | Expose backend workflows through thin request/response handlers without placing business rules in the API layer. | Planned |
| 8 | Frontend foundation | Initialize the React and TypeScript frontend structure, route layout, API client boundary, and frontend test setup. | Planned |
